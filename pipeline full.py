...
def plot_accuracy_curve(histo):
    import matplotlib.pyplot as plt
    plt.plot(histo.history['loss'])
    plt.plot(histo.history['mae'])
...
from keras.layers import Embedding, Input, LSTM, Dense, Dropout, Lambda, Flatten, Bidirectional, Conv2D, Conv1D, \
    MaxPooling1D, GlobalMaxPooling1D
from keras.models import Sequential, Model, load_model, model_from_config
import keras.backend as K


def get_model(Hidden_dim1=400, Hidden_dim2=128, return_sequences=True, dropout=0.5, recurrent_dropout=0.4,
              input_size=768, activation='relu', bidirectional=False):
    """Define the model."""
    model = Sequential()
    if bidirectional:
        ...
        model.add(LSTM(Hidden_dim1, dropout=0.4, recurrent_dropout=recurrent_dropout, input_shape=[1, input_size],
...
def get_model_CNN(Hidden_dim1=400, Hidden_dim2=128, return_sequences=True, dropout=0.5, recurrent_dropout=0.4,
                  input_size=768, output_dims=10380, activation='relu', bidirectional=False):
    """Define the model."""
...
#     #     ppb.RobertaModel, ppb.RobertaTokenizer, 'roberta-base')
#
#     # For DeBERTa:
#     # model_class, tokenizer_class, pretrained_weights = (
#     #      ppb.DebertaModel, ppb.DebertaTokenizer, 'microsoft/deberta-base')  # TFDebertaModel
#
#     # for Mobile Bert: paper
#     #model_class, tokenizer_class, pretrained_weights = (ppb.MobileBertModel, ppb.AutoTokenizer, 'google/mobilebert-uncased')
#
#     # For DistilBERT:
#     # model_class, tokenizer_class, pretrained_weights = (
...
#     # For Albert: paper
#     # model_class, tokenizer_class, pretrained_weights = (
#     #     ppb.AlbertModel, ppb.AutoTokenizer, 'albert-base-v2')
#
#     # For Electra: paper
#     model_class, tokenizer_class, pretrained_weights = (
#          ppb.ElectraModel, ppb.AutoTokenizer, 'google/electra-small-discriminator')
#
#     # For BERT:
...
#             # last_hidden_states_tr = outputs.last_hidden_state
# ##################################### try ^
#
...
#
#         history = lstm_model.fit(trainDataVectors, y_train, batch_size=batch_size, epochs=epoch, verbose = False)
...
#         #                                          learning_rate=0.1, loss='squared_error', max_depth=2, max_features=None,
#         #                                          max_leaf_nodes=None, min_impurity_decrease=0.0,
#         #                                          min_samples_leaf=1,
#         #                                          min_samples_split=2, min_weight_fraction_leaf=0.0,
#         #                                          n_estimators=1000, random_state=None,
#         #                                          subsample=1.0, verbose=0, warm_start=False)
#         gbr = ensemble.HistGradientBoostingRegressor(learning_rate=0.1, loss='squared_error', max_depth=2,
#                                                      max_leaf_nodes=None,
#                                                      min_samples_leaf=1,
#                                                      random_state=None,
...
#         # print(trainDataVectors.shape)
#         # print(y_train.shape)
#         tr_temp1 = trainDataVectors.reshape((trainDataVectors.shape[0], trainDataVectors.shape[2]))
#         # tr_temp1 = np.around(tr_temp1)
#         # np.nan_to_num(tr_temp1)
#         gbr.fit(tr_temp1, y_train)
#         # print(trainDataVectors.shape)
#         # print(y_train.shape)
#         # history = lstm_model.fit(trainDataVectors, y_train, batch_size=batch_size, epochs=epoch)
#         # plot_accuracy_curve(history)
#         te_temp1 = testDataVectors.reshape((testDataVectors.shape[0], testDataVectors.shape[2]))
#         y_pred = gbr.predict(te_temp1)
#         y_pred = np.around(y_pred)
#         np.nan_to_num(y_pred)
#         result = cohen_kappa_score(y_test.values, y_pred, weights='quadratic')
#         print("Kappa Score: {}".format(result))
...
for s in sets:
    ...
    all_sets_score.append(results)
print("scores on 8 sets: ", all_sets_score)
plot_qwk_scores_all_sets()
...
#     # print(trainDataVectors.shape)
#     # print(y_train.shape)
#     history = gbr.fit(trainDataVectors, y_train)
#     plot_accuracy_curve(history)
#     y_pred = gbr.predict(testDataVectors)
#     y_pred = np.around(y_pred)
#     np.nan_to_num(y_pred)
#     result = cohen_kappa_score(y_test.values, y_pred, weights='quadratic')
#     print("Kappa Score: {}".format(result))
